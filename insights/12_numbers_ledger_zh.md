...
> 本仓库最常被违反的一条规则是「不同工作的成功率禁止直接比大小」。这张账本把 26 个被反复引用的头条数字放进同一张表，逐条标注任务集、泛化定义、计分与聚合方式、每方法每任务的试验次数、训练期与评测期的干预 / 重试 / 权重更新、独立性与证据形式，并给出 notes 来源。用法：只有当任务、平台、计分方式、演示与更新预算等关键条件可比时，两个数字才能作性能比较；其余情况只能是定性定位。notes 未说明的项一律写「notes 未说明」，不写「无」。
